...
// 业务术语表：统一注入两个提示词，保证提取/改写使用规范表述
const BUSINESS_GLOSSARY = `业务术语（输出必须使用规范表述）：
- 两地牌指标分两种：「期牌」=新申请指标，需排队约 3~4 个月（类似买房期房）；「现牌」=已申请好的现成指标，多花钱、直接过户、无需等待
- 客户口语常把"现牌"说成/写成"限牌"，遇到"限牌"一律规范为「现牌」
- 深圳湾、莲塘、沙头角、港珠澳为口岸名称
- 需求描述控制在 20~60 字：去掉口语和过程性描述，但车型/年份/颜色、口岸、期牌还是现牌、预算、时间、数量等关键信息一个都不能丢`;
...
- 有明确诉求 → {"has_needs": true, "needs": "凝练、规范的需求描述（20~60字）", "reason": "一句话依据"}
